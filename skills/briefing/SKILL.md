...
And it is a floor for the *easy* case. `4.8cqw` is calibrated for text against text, where both edges of the gutter are ragged. Put a photograph, a chart or a bordered call-out opposite that text and one edge becomes a hard vertical rule the height of the row; the last words of every line then read as touching it, and the pair looks crowded rather than adjacent. Add **`.cols--figure`** (`6.4cqw`) for that pairing – it is what the title slide has always done, with a `9cqw` column gap for a headline against a card. `md-to-deck.mjs` applies it automatically.
...
**A reveal is a slide, so build it like one.** A `.detail-layer` is a full-slide panel outside the scroll path – no page number, no nav dot, unreachable by paging. Use it for material a reader may want and most will not: a derivation, a caveat, the numbers behind a claim. Without it, that material either takes a slide of its own that everyone has to scroll past, or it is left out. Inside the layer, use the same components you would on a slide (`h2`, `.cols`, `.net`, a closing `.punch`) and never a bare `<p>` – those fall back to 16px and stop scaling. The layer must be the **next sibling** of its strip; on a slide with two reveals that is the only unambiguous pairing, and the runtime warns when it has to guess. The layer is also the slide nobody proof-reads, so check it for overflow explicitly – the audit only sees what is on screen.
